import { TVRoot } from '@/types/types'
...
export function TopRatedTVSlider() {
  const { data, isLoading } = useQuery<TVRoot>({
        ...
        <Carousel className="w-full">
          <CarouselContent className="px-8">
            {data?.results.map((result) => (
              ...
              >
                <Link to={`/tv/${result.id}`} className="space-y-2 group">
                  <CarouselImageBlur
                    dataForChange={data}
