...
export function Movies() {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteQuery<MoviesRoot>({
      queryKey: ['trending-movies-list', isTopRated],
      queryFn: async ({ pageParam }) => {
        try {
          const response = await fetch(
            isTopRated
              ? `https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=pt-BR&page=${pageParam}`
              : `https://api.themoviedb.org/3/trending/movie/week?api_key=${apiKey}&language=pt-BR&page=${pageParam}`
          )
